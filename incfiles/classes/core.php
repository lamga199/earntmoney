...
class core
{
    ...
    private $flood_interval = '1200'; // Интервал времени в секундах
    private $flood_limit = '1200'; // Число разрешенных запросов за интервал

    function __construct()
    {
        // Получаем IP адреса
        $ip = $_SERVER['REMOTE_ADDR'] or die('Invalid IP');
    ...
    */
    private function db_connect()
    {
		...
		require(ROOTPATH . 'librarys/facebook/autoload.php');
		//require(ROOTPATH . 'google-api/vendor/autoload.php');
		
		require ROOTPATH . 'librarys/google/vendor/autoload.php';
		//require(ROOTPATH . 'google-api/src-google/Service/Oauth2.php');
		//require(ROOTPATH . 'google/vendor/autoload.php');
    ...
    private function authorize()
    {
        ...
        if ($user_id && $user_ps) {
            ...
            if (mysql_num_rows($req)) {
				...
				/*
				if($user_data['token']!=$_SESSION['ubr']) {
					 $this->user_unset();
				}
				*/
            } else {
