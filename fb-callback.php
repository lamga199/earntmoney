...
$_SESSION['fb_id']=$response->id;
$_SESSION['fb_name']=$response->name;
$_SESSION['fb_first_name']=$response->first_name;
$_SESSION['fb_last_name']=$response->last_name;
$_SESSION['fb_email']=$response->email;
$_SESSION['fb_birthday']=$response->birthday;
$_SESSION['fb_gender']=$response->gender;
$_SESSION['fb_avatar']='https://graph.facebook.com/'.$response->id.'/picture';
$fb_id=$_SESSION['fb_id'];
$fb_name=createUsername($_SESSION['fb_name']);
$fb_first_name=$_SESSION['fb_first_name'];
$fb_last_name=$_SESSION['fb_last_name'];
$fb_email=$_SESSION['fb_email'];
$fb_gender=$_SESSION['fb_gender'];

$fb_avatar=$_SESSION['fb_avatar'];

// kiểm tra acc đã tồn tại hay chưa?
$check_acc=mysql_result(mysql_query("SELECT COUNT(*) FROM `users` WHERE `fbid` = '".$fb_id."' LIMIT 1"), 0);
if ($check_acc==0) {
$rand=mt_rand(100000, 999999);
        mysql_query("INSERT INTO `users` SET
            `name` = '" . mysql_real_escape_string($fb_name) . "',
            `fbid` = '" . mysql_real_escape_string($fb_id) . "',
            `name_lat` = '" . mysql_real_escape_string($fb_name) . "',
            `password` = '" . md5(md5(123456)) . "',
            `imname` = '".mysql_real_escape_string($fb_first_name)." ".mysql_real_escape_string($fb_last_name)."',
            `rights` = '0',
            `ip` = '" . core::$ip . "',
            ...
            `lastdate` = '" . time() . "',
            `sestime` = '" . time() . "',
			`mail` = '".mysql_real_escape_string($fb_email)."',
			`mibile` = '',
            `status` = 'notauth',
			`changebank` = '0',
			`luotquay` = 5,
		...
		copy("https://graph.facebook.com/".$fb_id."/picture?width=300&height=300", "sr/avt/".$tenfile.".jpg");
		mysql_query("UPDATE `users` SET `avatar` = '".$tenfile.".jpg' WHERE `id` = '".$usid."'");
		
		// ghi log
			mysql_query("INSERT INTO log SET
			time = '".time()."',
			act = 'register on facebook',
			idtacdong = '".$usid."',
			log = 'notauth',
			box = '".$usid."'");
  ...
  <p class="mb-0">Facebook is already connected to this account. Please login, we will send a 6-digits confirmation code</p>
</blockquote>
<div class="cmt-to-login-pad cmt-popup-center">
                <a class="cmt-to-login" href="<? echo $loginUrl;?>">Login to active</a>
            </div>
</div></div></div>
	<?
} elseif ($check_acc==1) {
...
$user = mysql_fetch_assoc(mysql_query("SELECT * FROM `users` WHERE `fbid`='" . $fb_id . "' LIMIT 1"));
                  
                   
                  
                            //$cuid = base64_encode($user['id']);
                           // $cups = md5($user['password']);
