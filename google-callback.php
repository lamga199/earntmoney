...
<?php
	...
	$_SESSION['google_first_name']=$userinfo['familyName'];
	$_SESSION['google_last_name']=$userinfo['givenName'];
	$_SESSION['google_email']=$userinfo['email'];	
	$_SESSION['google_gender']=$userinfo['gender'];
	$_SESSION['google_avatar']=$userinfo['picture'];
	
	$google_id=$_SESSION['google_id'];
	$google_name=createUsername($_SESSION['google_name']);
	$google_first_name=$_SESSION['google_first_name'];
	$google_last_name=$_SESSION['google_last_name'];
	$google_email=$_SESSION['google_email'];
	$google_gender=$_SESSION['google_gender'];
	$google_avatar=$_SESSION['google_avatar'];
	
	$check_acc=mysql_result(mysql_query("SELECT COUNT(*) FROM `users` WHERE `google` = '".$google_id."' LIMIT 1"), 0);
	
if ($check_acc==0) {
$rand=mt_rand(100000, 999999);
        mysql_query("INSERT INTO `users` SET
            `name` = '" . mysql_real_escape_string($google_name) . "',
            `google` = '" . mysql_real_escape_string($google_id) . "',
            `name_lat` = '" . mysql_real_escape_string($google_name) . "',
            `password` = '" . md5(md5(123456)) . "',
            `imname` = '".mysql_real_escape_string($google_first_name)." ".mysql_real_escape_string($google_last_name)."',
            `rights` = '0',
            `ip` = '" . core::$ip . "',
            ...
            `lastdate` = '" . time() . "',
            `sestime` = '" . time() . "',
			`mail` = '".mysql_real_escape_string($google_email)."',
			`mibile` = '',
            `status` = 'notauth',
			`changebank` = '0',
			`luotquay` = 5,
  ...
  <p class="mb-0">Google is already connected to this account. Please login, we will send a 6-digits confirmation code</p>
</blockquote>
<div class="cmt-to-login-pad cmt-popup-center">
                <a class="cmt-to-login" href="<? echo $loginUrl_google;?>">Login to active</a>
            </div>
</div></div></div>
	<?
} elseif ($check_acc==1) {

			$user = mysql_fetch_assoc(mysql_query("SELECT * FROM `users` WHERE `google`='" . $google_id . "' LIMIT 1"));
