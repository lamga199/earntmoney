...
<?php
 ...
 if(isset($_POST['submit'])) {
	 ...
	 $checkon=mysql_fetch_assoc(mysql_query("SELECT * FROM `payinternet` WHERE `id` = '".$check."'"));
	...
	mysql_query("INSERT INTO report SET
	 ...
	 if($duyet=='ok') {
		 mysql_query("UPDATE `payinternet` SET
	`timeduyet` = '".time()."',
	`admin` = '".$user_id."',
	`status` = 'done'
	WHERE `id` = '".$checkon['id']."'");
	 mysql_query("UPDATE `log` SET `status` = 'done' WHERE `boxid` = '".$checkon['id']."'");
	 $note = 'Congratulations . Internet bill payment request is successful';
	...
	WHERE `id` = '".$checkon['id']."'");
		...
		$note = 'Error: Internet payment request fails, your balance will be refunded';
	 mysql_query("INSERT INTO `news` SET 
	 `time` = '".time()."',
	 `name` = '".$note."',
	 `color` = 'red',
...
<td class="bang">Duyệt</td>
  ...
  <?php
	 ...
	 $req=mysql_query("SELECT * FROM `payinternet` ORDER BY `time` DESC LIMIT $start,$kmess");
	 $i=1;
