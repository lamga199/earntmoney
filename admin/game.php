...
<div style="background:#000;color:#fff;width:120px;padding:5px;margin:auto">Duyệt thanh toán game</div>
</div>
...
<?php
 ...
 if(isset($_POST['submit'])) {
	 ...
	 $checkon=mysql_fetch_assoc(mysql_query("SELECT * FROM `paygame` WHERE `id` = '".$check."'"));
	...
	mysql_query("INSERT INTO report SET
	 ...
	 if($duyet=='ok') {
		 mysql_query("UPDATE `paygame` SET
	`timeduyet` = '".time()."',
	`admin` = '".$user_id."',
	`status` = 'done'
	WHERE `id` = '".$checkon['id']."'");
	 mysql_query("UPDATE `log` SET `status` = 'done' WHERE `boxid` = '".$checkon['id']."'");
	 	...
	 	$textnote='Deposit into GAME successfully. Please login to your game and join now';
	...
	WHERE `id` = '".$checkon['id']."'");
	 ...
	 $textnote='Error! The GAME deposit request has been declined, your balance will be refunded. Please check again';
		mysql_query("INSERT INTO news SET
			time = '".time()."',
			color = 'red',
			`type` = 'note',
...
<td class="bang">Duyệt</td>
  ...
  <?php
	 ...
	 $req=mysql_query("SELECT * FROM `paygame` ORDER BY `time` DESC LIMIT $start,$kmess");
	 $i=1;
