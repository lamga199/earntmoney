...
<div style="background:#000;color:#fff;width:120px;padding:5px;margin:auto">Duyệt thanh toán điện thoại</div>
</div>
...
<?php
 ...
 if(isset($_POST['submit'])) {
	 ...
	 $checkon=mysql_fetch_assoc(mysql_query("SELECT * FROM `paycard` WHERE `id` = '".$check."'"));
	...
	mysql_query("INSERT INTO report SET
	 ...
	 if($duyet=='ok') {
		 mysql_query("UPDATE `paycard` SET
	`timeduyet` = '".time()."',
	`admin` = '".$user_id."',
	`status` = 'done'
	WHERE `id` = '".$checkon['id']."'");
	 ...
	 $textnote='Congratulations Your mobile recharge request has been processed. Check your SIM balance';
	...
	WHERE `id` = '".$checkon['id']."'");
	  ...
	  $textnote='Error: The mobile recharge request failed, you will be refunded your balance';
		mysql_query("INSERT INTO news SET
			time = '".time()."',
			color = 'red',
			`type` = 'note',
...
<td class="bang">Mạng</td>
<td class="bang">Số điện thoại</td>
...
<td class="bang">Duyệt</td>
  ...
  <?php
	 ...
	 $req=mysql_query("SELECT * FROM `paycard` ORDER BY `time` DESC LIMIT $start,$kmess");
	 $i=1;
