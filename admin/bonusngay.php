...
<?php
 $total = mysql_result(mysql_query("SELECT COUNT(*) FROM `log` WHERE `idtacdong` = '".$idnext."' AND `act` = 'add coinday diem danh'"), 0);
?>
...
<td class="bang">Status</td>
  ...
  <?php
	 ...
	 $req=mysql_query("SELECT * FROM `log` WHERE `idtacdong` = '".$idnext."'  AND `act` = 'add coinday diem danh' ORDER BY `time` DESC LIMIT $start,$kmess");
	 $i=1;
